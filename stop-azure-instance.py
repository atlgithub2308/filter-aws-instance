...
print('Stopping {} Azure Virtual machine(s)'.format(len(resource_ids)))
...
for resource_id in resource_ids:
  ...
  print('Stopping Azure Virtual Machine {0} in Resource Group {1}'.format(vm_name, resource_group_name))
  async_vm_operation = compute_client.virtual_machines.begin_power_off(resource_group_name, vm_name)
...
if wait:
  for async_vm_operation in vm_handle_list:
    print('Waiting for Azure Virtual Machine to be stopped.')
    async_vm_operation.wait()

print('All specified Azure Virtual Machines are stopped.')
